    ...
    <div class="container py-5">
      ...
      >
        <div class="mb-3 mb-md-0">
          <h2 class="fw-bold text-dark m-0">{{ 'adminUsers.title' | translate }}</h2>
          <p class="text-muted small m-0">{{ 'adminUsers.description' | translate }}</p>
        </div>
      ...
      @if (isLoading()) {
        <div class="text-center py-5">
          ...
          <p class="text-muted mt-2">{{ 'adminUsers.loading' | translate }}</p>
        </div>
      } @else {
        @if (users().length > 0) {
            ...
            <div class="table-responsive">
              <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                  <tr>
                    <th class="ps-4">{{ 'adminUsers.userProfile' | translate }}</th>
                    <th>{{ 'adminUsers.accountRole' | translate }}</th>
                    <th>{{ 'adminUsers.status' | translate }}</th>
                    <th class="text-end pe-4">{{ 'adminUsers.actions' | translate }}</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr [class.table-secondary]="user.isDeleted">
                      ...
                      <td>
                        ...
                        >
                          <option value="customer" [selected]="user.role === 'customer'">
                            {{ 'adminUsers.customer' | translate }}
                          </option>
                          <option value="seller" [selected]="user.role === 'seller'">
                            {{ 'adminUsers.seller' | translate }}
                          </option>
                          <option value="admin" [selected]="user.role === 'admin'">
                            {{ 'adminUsers.admin' | translate }}
                          </option>
                        </select>
                      </td>
                      <td>
                      ...
                      <td class="text-end pe-4">
                        ...
                        <button
                          ...
                          [class.btn-success]="user.isRestricted"
                          [class.btn-outline-danger]="!user.isRestricted"
                          (click)="openConfirmModal('Ban/Unban User', user)"
                          [disabled]="user.isDeleted"
                        >
                          {{
                            (user.isRestricted ? 'adminUsers.unban' : 'adminUsers.ban') | translate
                          }}
                        </button>
                        <button
          ...
          <div class="text-center py-5">
            ...
            <p class="text-muted mt-3">{{ 'adminUsers.noUsers' | translate }}</p>
          </div>
        }
      }
      ...
      <div class="position-fixed top-0 end-0 p-3" style="z-index:2000">
        @for (toast of toasts; track $index) {
          <div
            class="toast show align-items-center text-bg-{{ toast.type }} border-0 mb-2 shadow-lg"
          >
            <div class="d-flex">
            ...
            <div class="modal-content border-0 shadow-lg">
              <div class="modal-header bg-warning text-dark">
                <h5 class="modal-title fw-bold">
                  <i class="bi bi-award me-2"></i>{{ 'adminUsers.loyaltyPointsTitle' | translate }}
                </h5>
                <button type="button" class="btn-close" (click)="closePointsModal()"></button>
              </div>
              <div class="modal-body p-4">
                <p>
                  {{ 'adminUsers.grantTo' | translate }} <strong>{{ selectedUser?.name }}</strong>
                </p>
                <input
                  type="number"
                  class="form-control form-control-lg rounded-3"
                  [placeholder]="'adminUsers.amountPlaceholder' | translate"
                  [(ngModel)]="pointsToAdd"
                />
              </div>
              <div class="modal-footer border-0">
                <button class="btn btn-light px-4" (click)="closePointsModal()">
                  {{ 'adminUsers.cancel' | translate }}
                </button>
                <button class="btn btn-warning px-4 fw-bold" (click)="submitPoints()">
                  {{ 'adminUsers.grantPoints' | translate }}
                </button>
              </div>
            </div>
          </div>
          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
              <div
                class="modal-header"
                [ngClass]="
                  confirmTitle.includes('Delete') ? 'bg-danger text-white' : 'bg-primary text-white'
                "
              >
              ...
              <div class="modal-footer border-0 justify-content-center">
                <button class="btn btn-light px-4" (click)="closeConfirmModal()">
                  {{ 'adminUsers.cancel' | translate }}
                </button>
                <button
                  class="btn px-4 fw-bold"
                  [ngClass]="confirmTitle.includes('Delete') ? 'btn-danger' : 'btn-primary'"
                  (click)="confirmAction()"
                >
                  {{ 'adminUsers.proceed' | translate }}
                </button>
              </div>
            </div>
          </div>
...
export class AdminUsersComponent implements OnInit {
    ...
    this.adminService.updateUserRole(userId, newRole).subscribe({
      next: () => {
        ...
        this.users.update((list) =>
          list.map((u) => (u._id === userId ? { ...u, role: newRole } : u)),
        );
      },
      error: (err) => {
  ...
  submitPoints() {
    if (this.pointsToAdd && !isNaN(this.pointsToAdd)) {
      this.adminService.grantLoyaltyPoints(this.selectedUser._id, this.pointsToAdd).subscribe({
        next: () => {
          this.showToast(
            `${this.pointsToAdd} points added to ${this.selectedUser.name}`,
            'success',
          );
  ...
  openConfirmModal(action: string, user: any) {
    ...
    this.confirmMessage =
      action === 'Delete User'
        ? `Are you sure? This will delete ${user.name} permanently.`
        : `Toggle restriction for ${user.name}?`;
